...
class SleepTime extends React.Component {
	...
	render() {
		...
		return (
			...
			<div id="chart" style={{width: '100%', padding: 10, paddingBottom: 40}}>
				{this.props.sleepData.map(s => {
                    ...
                    return <ReactApexChart key={`${s.y[0]}${s.y[1]}`} options={options} series={series} type="rangeBar" height={70} style={{marginBottom: -55, marginTop: -40, width: '95%'}}/>
				})}
			</div>
		);
	}
